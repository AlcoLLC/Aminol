document.addEventListener('DOMContentLoaded', function () {
    ...
    function setActiveLinks() {
        ...
        const dropdownParents = document.querySelectorAll('.dropdown > a, .mobile-dropdown > .mobile-dropdown-head');
        dropdownParents.forEach(parent => {
            parent.classList.remove('active');
        });

        // Dropdown linklərini yoxla
        const dropdownLinks = document.querySelectorAll('.dropdown-content a[href], .mobile-dropdown-content a[href]');
        ...
        if (!activeDropdownFound) {
            const regularNavLinks = document.querySelectorAll('.navbar a[href]:not(.dropdown-content a), .mobile-menu a[href]:not(.mobile-dropdown-content a)');
    ...
    function handleSpecialDropdownCases(currentPath) {  
        ...
        if (marketsPaths.includes(currentPath)) {
            const marketsDropdown = Array.from(document.querySelectorAll('.dropdown > a'))
                .find(link => link.textContent.trim().toLowerCase().includes('market'));

            if (marketsDropdown) {
                marketsDropdown.classList.add('active');
            }

            const mobileMarketsDropdown = Array.from(document.querySelectorAll('.mobile-dropdown-head'))
                .find(link => link.textContent.trim().toLowerCase().includes('market'));
            ...
            const activeDropdownLink = document.querySelector(`.dropdown-content a[href="${currentPath}"], .mobile-dropdown-content a[href="${currentPath}"]`);
        ...
        if (servicesPaths.includes(currentPath)) {
            const servicesDropdown = Array.from(document.querySelectorAll('.dropdown > a'))
                .find(link => link.textContent.trim().toLowerCase().includes('service'));

            if (servicesDropdown) {
                servicesDropdown.classList.add('active');
            }

            const mobileServicesDropdown = Array.from(document.querySelectorAll('.mobile-dropdown-head'))
                .find(link => link.textContent.trim().toLowerCase().includes('service'));
